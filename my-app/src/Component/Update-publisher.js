...
const UpdatePublisher = ({ publisher }) => {
  ...
  const [publisherName, setPublisherName] = useState(publisher ? publisher.name || '' : '');

  // Check if publisher is defined before using it
  if (!publisher) {
    console.error('Publisher is undefined.');
    return null; // or render an error message or redirect
  }
  ...
  const handleSubmit = (event) => {
    ...
    if (publisherName.trim() !== '') {
      ...
      // You can send a request to update the publisher or perform other actions
    } else {
